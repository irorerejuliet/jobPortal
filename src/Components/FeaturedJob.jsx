...
const FeaturedJob = () => {
  return (
    <div className="mt-32">
      ...
      <div className="md:px-0 px-4 grid md:grid-cols-3 grid-cols-1 space-y-6">
        {featuredData.map(
          ({ id, image, name, title, description, city, site }) => (
            <div className="shadow-lg md:w-[384px] w-[358px] text-center border border-gray-300 rounded-xl py-8 px-12">
              <div className="text-center ml-[7rem]" key={id}>
                <img src={image} alt="iosdeveloper" className="w-[64px]" />
              </div>
              <p className="text-base font-bold text-primary py-3">
                {name}
              </p>
